(function ($, api) {
  ...
  api.sliderField = api.Fields.extend({
    attachEvent: function () {
      ...
      $(this.element).find('.jeg-slider-reset').click(function () {
  ...
  api.selectField = api.Fields.extend({
    ...
    renderSelect: function (options) {
      ...
      if (( field.params.multiple && field.params.multiple > 1 ) || field.params.ajax) {
        ...
        // Single.
        input = $(field.element).find('select');
        setting = {
  ...
  api.imageField = api.Fields.extend({
    ready: function () {
      ...
      field.createMediaUploadInstance()
    ...
    createMediaUploadInstance: function () {
      ...
      field.mediaUpload.on('insert', function () {
        var json = field.mediaUpload.state().get('selection').first().toJSON()
        field.addImage(json)
  ...
  api.repeaterField = api.Fields.extend({
    ...
    getValue: function () {
      ...
      return $.isArray($(this.element).find('.data-setting').attr('value')) ? $(this.element).find('.data-setting').attr('value') : JSON.parse(decodeURIComponent($(this.element).find('.data-setting').attr('value')));
