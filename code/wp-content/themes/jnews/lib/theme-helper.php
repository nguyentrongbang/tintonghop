...
if ( ! function_exists( 'jnews_create_button' ) ) {
	function jnews_create_button( $value ) {
		...
		?>
        <a href="<?php echo esc_attr( $button_link ) ?>" class="btn <?php echo esc_attr( $button_form ); ?>"
		...
		if ( is_admin() ) {
			...
			if ( ! empty( $log_version ) ) {
				if ( version_compare( $current_version, $log_version['current_version'], '>' ) ) {
					update_option( 'jnews_theme_version_log', array(
...
if ( ! function_exists( 'jeg_get_post_date' ) ) {
	function jeg_get_post_date( $format = '', $post = null ) {
		...
		} else if (get_theme_mod( 'jnews_global_post_date', 'modified' ) === 'both'){
            /*Adding option to show both dates*/
            if (get_the_date( $format, $post ) == get_the_modified_date( $format, $post )){
                return get_the_date( $format, $post );
            }
            else{
                return get_the_date( $format, $post ).' - Updated on '.get_the_modified_date( $format, $post );
            }
        }
        ...
        if (defined('WPB_VC_VERSION')) {
            ...
            if ($id) {
                ...
                if (!empty($post_custom_css)) {
                    ...
                    wp_add_inline_style('jeg-dynamic-style', $post_custom_css);
                }
            }
        } else {
            return;
        ...
        if (defined('WPB_VC_VERSION')) {
            ...
            if ($id) {
                ...
                if (!empty($shortcodes_custom_css)) {
                    ...
                    wp_add_inline_style('jeg-dynamic-style', $shortcodes_custom_css);
                }
            }
        } else {
            return;
...
if ( ! function_exists( 'jeg_render_builder_content' ) ) {
	function jeg_render_builder_content( $page_id ) {
		if ( defined( 'ELEMENTOR_VERSION' ) && \Elementor\Plugin::$instance->db->is_built_with_elementor( $page_id ) ) {
			...
			jnews_admin_topbar_menu( [ $frontend, 'add_menu_in_admin_bar' ], 200 );
