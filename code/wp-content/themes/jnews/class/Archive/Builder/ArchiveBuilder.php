...
Class ArchiveBuilder {
	...
	public function render_item_list( $template ) {
		$name                = isset( $template['name'] ) ? esc_html( $template['name'] ) : esc_html( __( 'No title', 'jnews' ) );
	...
	protected function override_archive_template_option( $archive ) {
		...
		foreach ( $keys as $key => $label ) {
			add_filter( "theme_mod_jnews_{$archive}_" . $label, function ( $value ) use ( $self, $key, $archive ) {
				$term = '';
