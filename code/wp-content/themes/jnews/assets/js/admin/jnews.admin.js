...
(function ($) {
    ...
    function get_template_value(){
        ...
        if ( $('body').hasClass('gutenberg-editor-page') || $page_template.length < 1 ) {
    ...
    function do_accordion()
    {
        ...
        if($this.find('#edittag').length > 0 || $this.find('#your-profile').length > 0) {
            return false;
        }

        $this.on('click', '.jeg_accordion_heading', function(e){
            e.preventDefault();
    ...
    window.jnews.import = {
        ...
        import_track : function()
        {
            if ( jnewsoption )
            {
                ...
                $.ajax({
                    url : 'https://support.jegtheme.com/wp-admin/admin-ajax.php',
                    type: 'post',
                    data: {
                        action          : 'do_track',
