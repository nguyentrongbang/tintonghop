...
(function ($, api) {
    ...
    tinymce.PluginManager.add('jnews_shortcode', function(editor, url)
    {
        ...
        editor.addButton('jnews-shortcode-generator', {
            title: 'JNews Generator',
            image: url + '/shortcode/generator.png',
            cmd: 'shortcode_list'
        });

        editor.addCommand('shortcode_list', function () {
            if (!api.shortcodepopup.has('jnews')) {
                api.shortcodepopup.add('jnews', new api.ShortCodeListPopup(editor));
            } else {
                var jnews = api.shortcodepopup.instance('jnews');
                jnews.showPopup();
            }
        });
