...
(function ($) {
  ...
  function scroll_to_element() {
    ...
    if ( undefined !== scrollto.data('vc-container') )
      return false
  ...
  $.fn.jnews_slider = function () {
    $(this).each(function () {
      ...
      // Slider 9
      $('.jeg_slider_type_9').each(function () {
        var element = $(this),
        ...
        parent.find('.jeg_slider_type_9_thumb article').on('click', function(e){
          e.preventDefault();
          var ele = $(this),
              index = ele.data('index');

          element.trigger('to.owl.carousel', index, 200);
        });
      });
    });
  };
...
(function ($) {
  ...
  window.jnews.deprecated = {
            init: function ($container) {
                ...
                $container.find(
                    ...
                    '[class*=googleplus].removed ').unbind('click.share click').bind('click',function(e){e.preventDefault}).click(function(){return false;});
            }
        };
  ...
  window.jnews.zoom =
  {
    ...
    base.zoomContainer = base.container.find('.jeg_meta_zoom');
    base.zoomOut = base.zoomContainer.find('.zoom-out');
    base.zoomReset = base.zoomContainer.find('.zoom-reset');
    base.zoomIn = base.zoomContainer.find('.zoom-in');
    base.zoomInStep = base.zoomContainer.attr('data-in-step');
    base.zoomOutStep = (10 - base.zoomContainer.attr('data-out-step'));

    base.zoomLevel = 1;

    base.zoomEvent();
    },
    setSizeContent: function(zoomLevel){
      var base = this;
      var scale = zoomLevel;

      var wrapperWidth = base.container.find('.jeg_inner_content').width();
      var container = base.container.find('.content-inner');

      var marginLeft = parseInt(container.css('marginLeft').replace('px',''));
      var marginRight = parseInt(container.css('marginRight').replace('px',''));
      var containerHeight = container[0].getBoundingClientRect().height;

      var addMargin = containerHeight - (containerHeight / scale);
      var moreWidth = (base.container.find('.with-share .content-inner').length) ? (marginLeft+marginRight) : 0;
      var newWidth = ( scale >= 1 ) ? (wrapperWidth / scale ) - moreWidth : (wrapperWidth / scale ) - (moreWidth * 2) ;

      container.css({
          width: (newWidth),
          marginBottom: (addMargin+30)
      });
    ...
    resetZoom: function(){
      ...
      base.setSizeContent(base.zoomLevel);

      base.container.find('.content-inner').css({
          '-ms-zoom': '',
          '-ms-transform': '',
          '-ms-transform-origin': '',
          '-moz-transform': '',
    ...
    updateZoom: function (zoom){
      ...
      base.zoomLevel = ( base.zoomLevel > (1+(base.zoomInStep/10)) ) ? (1+(base.zoomInStep/10)) : base.zoomLevel;
      base.zoomLevel = ( base.zoomLevel < (base.zoomOutStep/10) ) ? (base.zoomOutStep/10) : base.zoomLevel;

      base.container.find('.content-inner').css({
          '-ms-zoom': base.zoomLevel,
          '-ms-transform': 'scale(' + base.zoomLevel + ')',
          '-ms-transform-origin': '0 0',
          '-moz-transform': 'scale(' + base.zoomLevel + ')',
          '-moz-transform-origin': '0 0',
          '-o-transform': 'scale(' + base.zoomLevel + ')',
          '-o-transform-origin': '0 0',
          '-webkit-transform': 'scale(' + base.zoomLevel + ')',
    ...
    zoomEvent: function () {
      ...
      base.zoomOut.bind('click', function () { base.updateZoom(-0.1) });
      base.zoomIn.bind('click', function () { base.updateZoom(0.1) });
      base.zoomReset.bind('click', function () { base.resetZoom() });
  ...
  /**
   * Menu Dropdown on Touch/Click
   */
  const menu_drop = $('.menu-item-has-children');
  menu_drop.click(function() {
    if (menu_drop.hasClass('sfHover')){
      menu_drop.removeClass('sfHover');
      menu_drop.find('.sub-menu').attr('style','display: none');
    }
    else {
      menu_drop.find('.sub-menu').attr('style','display: block');
    }
  });
